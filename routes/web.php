...
use App\Http\Controllers\BuildingModuleController;
...
use App\Http\Controllers\Finance\WalkinPaymentController;
...
use App\Http\Controllers\Restaurant\BarController;
use App\Http\Controllers\Restaurant\BuffetController;
use App\Http\Controllers\Restaurant\KitchenController;
use App\Http\Controllers\Restaurant\KitchenStockController;
...
Route::middleware(['auth', 'building.scope'])->group(function () {
    ...
    // Building module management (admin + manager for own building)
    Route::middleware(['role:admin,manager'])->group(function () {
        Route::resource('building-modules', BuildingModuleController::class);
    });
    ...
    Route::prefix('restaurant')->name('restaurant.')->middleware('role:restaurant_manager,manager,admin,waiter')->group(function () {
        ...
        Route::post('kitchen/tickets/{ticket}/preparing', [KitchenController::class, 'markPreparing'])->name('kitchen.preparing')
            ->middleware('role:restaurant_manager,manager,admin,supervisor');
        Route::post('kitchen/tickets/{ticket}/ready', [KitchenController::class, 'markReady'])->name('kitchen.ready')
            ->middleware('role:restaurant_manager,manager,admin,supervisor');

        // ── Bar Queue & Tabs (bar_tender, restaurant_manager, manager, admin, supervisor) ──
        Route::get('bar/queue', [BarController::class, 'queue'])->name('bar.queue')
            ->middleware('role:bar_tender,restaurant_manager,manager,admin,supervisor');
        Route::get('bar/tabs', [BarController::class, 'tabs'])->name('bar.tabs')
            ->middleware('role:bar_tender,restaurant_manager,manager,admin,supervisor');
        Route::post('bar/tickets/{ticket}/preparing', [BarController::class, 'markPreparing'])->name('bar.preparing')
            ->middleware('role:bar_tender,restaurant_manager,manager,admin,supervisor');
        Route::post('bar/tickets/{ticket}/ready', [BarController::class, 'markReady'])->name('bar.ready')
            ->middleware('role:bar_tender,restaurant_manager,manager,admin,supervisor');

        // ── Menu Item Options API ─────────────────────────────────────────
        Route::get('menu-items/{menuItem}/options', [MenuItemController::class, 'options'])->name('menu.items.options');
    ...
    Route::prefix('finance')->name('finance.')->group(function () {
        ...
        Route::post('walkin-payment/process', [WalkinPaymentController::class, 'process'])
        ...
        Route::get('walkin-payment/status/{reference}', [WalkinPaymentController::class, 'status'])
            ->name('walkin-payment.status')
            ->middleware('role:front_desk,laundry_manager,supervisor,bar_tender,restaurant_manager,manager,admin');
        Route::get('walkin-payment/callback/{transaction}', [WalkinPaymentController::class, 'callback'])
        ...
        Route::get('refunds/payment/{payment}', [RefundController::class, 'showPayment'])->name('refunds.payment')
            ->middleware('role:manager,store_manager');
        Route::post('refunds/payment/{payment}', [RefundController::class, 'processPaymentRefund'])->name('refunds.payment.process')
            ->middleware('role:manager');
        Route::get('refunds/walkin/{transaction}', [RefundController::class, 'showWalkin'])->name('refunds.walkin')
            ->middleware('role:manager,store_manager');
        Route::post('refunds/walkin/{transaction}', [RefundController::class, 'processWalkinRefund'])->name('refunds.walkin.process')
            ->middleware('role:manager');
        // API endpoints for validation
        Route::get('refunds/payment/{payment}/validate', [RefundController::class, 'validatePaymentRefund'])->name('refunds.payment.validate')
            ->middleware('role:manager,store_manager');
        Route::get('refunds/walkin/{transaction}/validate', [RefundController::class, 'validateWalkinRefund'])->name('refunds.walkin.validate')
            ->middleware('role:manager,store_manager');
    });
    ...
    Route::middleware(['role:manager,restaurant_manager'])->prefix('manager')->name('manager.')->group(function () {
        ...
        Route::prefix('kitchen-stock')->name('kitchen-stock.')->group(function () {
            ...
            Route::get('/{item}', [KitchenStockController::class, 'show'])->name('show');
            Route::get('/{item}/edit', [KitchenStockController::class, 'edit'])->name('edit');
            Route::put('/{item}', [KitchenStockController::class, 'update'])->name('update');
            Route::delete('/{item}', [KitchenStockController::class, 'destroy'])->name('destroy');
            Route::post('/{item}/movement', [KitchenStockController::class, 'recordMovement'])->name('record-movement');
        });
    });
    ...
    Route::middleware(['role:ACCOUNTANT,manager'])->prefix('accountant')->name('accountant.')->group(function () {
        ...
        Route::get('source/walkin-payment/{reference}', [WalkinPaymentController::class, 'status'])->name('source.walkin-payment.status');
    });
    ...
    Route::middleware(['role:admin,manager'])->group(function () {
        ...
        // Laundry Items
        Route::get('laundry-items/archived', [LaundryItemController::class, 'archived'])->name('laundry-items.archived');
        Route::patch('laundry-items/{laundryItem}/restore', [LaundryItemController::class, 'restore'])->name('laundry-items.restore');

        // Kitchen Stock
        Route::get('manager/kitchen-stock/archived', [KitchenStockController::class, 'archived'])->name('manager.kitchen-stock.archived');
        Route::patch('manager/kitchen-stock/{item}/restore', [KitchenStockController::class, 'restore'])->name('manager.kitchen-stock.restore');
    });
});
