    ...
    <form method="POST" action="{{ route('accountant.payments.allocate', $supplierPayment) }}" class="rounded-2xl bg-white p-6 shadow-sm">
        ...
        <div class="flex items-center justify-between gap-3">
            ...
            @if($supplierPayment->status !== 'posted')
                <button class="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white">{{ __('accountant.ap.save_allocations') }}</button>
            @endif
        </div>
                ...
                <tbody>
                    @forelse($payables as $payable)
                        ...
                        <tr class="border-b border-gray-50 last:border-0">
                            <td class="py-3"><a href="{{ route('accountant.payables.show', $payable) }}" class="font-semibold text-indigo-600 hover:text-indigo-700">{{ $payable->reference }}</a></td>
                            ...
                            <td class="py-3 text-right font-bold text-amber-700"><x-money :amount="$payable->balance + (float) $existing" /></td>
                            <td class="py-3 text-right">
                                <input type="number" step="0.01" min="0" name="allocations[{{ $payable->id }}]" value="{{ old('allocations.' . $payable->id, $existing) }}" class="w-32 rounded-xl border-gray-200 text-sm" {{ $supplierPayment->status === 'posted' ? 'disabled' : '' }}>
                            </td>
                        </tr>
                    @empty
