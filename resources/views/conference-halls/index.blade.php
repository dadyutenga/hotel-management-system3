{{-- resources/views/conference-halls/index.blade.php --}}
@extends('layouts.app')

@section('title', 'Conference Halls')
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-4 hover:shadow-xl transition-all">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-2xl font-extrabold text-secondary">{{ $halls->total() }}</div>
                    <div class="text-xs text-gray-500 font-medium">Total Halls</div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-4 hover:shadow-xl transition-all">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-2xl font-extrabold text-secondary">{{ $halls->where('status', 'available')->count() }}</div>
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-4 hover:shadow-xl transition-all">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-2xl font-extrabold text-secondary">{{ $halls->where('status', 'maintenance')->count() }}</div>
                    <div class="text-xs text-gray-500 font-medium">Maintenance</div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-4 hover:shadow-xl transition-all">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-2xl font-extrabold text-secondary">{{ $halls->sum('conference_bookings_count') }}</div>
                    <div class="text-xs text-gray-500 font-medium">Total Bookings</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-100">
            <thead class="bg-gradient-to-r from-blue-50 to-white">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-bold text-primary uppercase tracking-wider">Capacity</th>
                    <th class="px-6 py-4 text-left text-xs font-bold text-primary uppercase tracking-wider">Hourly Rate</th>
                ...
                <tr class="hover:bg-blue-50/50 transition-colors">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            <div class="w-10 h-10 bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl flex items-center justify-center text-white font-bold shadow-lg">
                                {{ substr($hall->name, 0, 1) }}
                            </div>
                            <div class="ml-3">
                                <div class="text-sm font-semibold text-secondary">{{ $hall->name }}</div>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center gap-1">
                            ...
                            <span class="text-sm font-medium text-secondary">{{ $hall->capacity }}</span>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="text-sm font-bold text-secondary">${{ number_format($hall->hourly_rate, 2) }}/hr</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-gradient-to-br from-blue-50 to-blue-100 text-primary">
                            {{ $hall->conference_bookings_count }} bookings
                        </span>
                    </td>
