        ...
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div class="flex items-center gap-4">
                ...
                <span class="px-2 py-1 text-xs rounded
                    ...
                    {{ $laundryOrder->customer_type === 'guest' ? 'Hotel Guest' : 'Walk-in' }}
                </span>
                @if($laundryOrder->isOverdue())
                <span class="px-2 py-1 text-xs rounded-full bg-red-100 text-red-700 font-medium">⚠ Overdue</span>
                @endif
            </div>
    ...
    <div id="settle-panel" class="hidden bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
        ...
        <form method="POST" action="{{ route('laundry.orders.settle', $laundryOrder) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                    <label class="block text-sm font-semibold text-secondary mb-2">Payment Method <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="block text-sm font-semibold text-secondary mb-2">Discount (optional)</label>
                    <div class="relative">
                ...
                <div id="booking-field" class="{{ $laundryOrder->customer_type === 'guest' ? '' : 'hidden' }}">
                    <label class="block text-sm font-semibold text-secondary mb-2">Booking ID</label>
            ...
            <div class="flex justify-end">
                <button type="submit" 
                    ...
                    Confirm Payment — {{ number_format($laundryOrder->total, 0) }} TZS
                </button>
            </div>
        </form>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="md:col-span-2 bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-white">
                <h3 class="text-sm font-bold text-primary uppercase tracking-wider">Order Items</h3>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-100">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-bold text-primary uppercase tracking-wider">Service</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-primary uppercase tracking-wider">Item</th>
                            <th class="px-6 py-3 text-right text-xs font-bold text-primary uppercase tracking-wider">Qty</th>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-bold text-primary uppercase tracking-wider">Notes</th>
                        </tr>
                    </thead>
                    ...
                    <tfoot class="bg-gray-50 border-t border-gray-100">
                        ...
                        <tr>
                            <td colspan="4"></td>
                            <td class="px-6 py-3 text-right text-sm text-gray-500">Discount</td>
                        ...
                        <tr>
                            <td colspan="4"></td>
                            <td class="px-6 py-3 text-right text-sm font-bold text-secondary">Total (TZS)</td>
        ...
        {{-- Order Info Sidebar --}}
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
            <h3 class="text-sm font-bold text-primary uppercase tracking-wider mb-4">Order Information</h3>
            <dl class="space-y-4">
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
            <h3 class="text-sm font-bold text-primary uppercase tracking-wider mb-4">Special Instructions</h3>
