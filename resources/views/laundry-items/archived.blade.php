        ...
        <table class="min-w-full divide-y divide-gray-100">
            <thead class="bg-gradient-to-r from-blue-50 to-white">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-bold text-primary uppercase tracking-wider">Price</th>
                    <th class="px-6 py-4 text-left text-xs font-bold text-primary uppercase tracking-wider">Category</th>
            ...
            <tbody class="bg-white divide-y divide-gray-100">
                ...
                <tr class="hover:bg-blue-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="text-sm font-bold text-secondary">{{ number_format($item->price, 2) }}</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-secondary">{{ $item->category ?? 'N/A' }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-500">{{ $item->deleted_at?->format('M d, Y H:i') }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <form method="POST" action="{{ route('laundry-items.restore', $item) }}" class="inline">
                            @csrf
                            @method('PATCH')
                            <button type="submit"
