...
@section('content')
<div class="max-w-2xl" x-data="{ productType: '{{ old('product_type', $product->product_type ?? 'normal') }}' }">
        ...
        <form method="POST" action="{{ route('store.products.update', $product) }}" x-data="productVarieties()" enctype="multipart/form-data">
            ...
            <div class="mt-6">
                <div class="flex items-center justify-between mb-3">
                    <h3 class="text-sm font-semibold text-gray-700">Varieties</h3>
                ...
                <template x-for="(v, idx) in varieties" :key="idx">
                    <div class="grid grid-cols-3 gap-2 mb-2 items-end">
                        <div>
                            <label class="text-xs text-gray-500">Label (e.g. 500ml)</label>
...
function productVarieties() {
    return {
        varieties: @json($product->varieties ?? []),
        init() {
            this.syncJson();
