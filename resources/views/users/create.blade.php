...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100">
        ...
        <div class="px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-white rounded-t-2xl">
            <h2 class="text-xl font-extrabold text-secondary">{{ __('users.create_user') }}</h2>
            <p class="text-sm text-gray-500 mt-1">{{ __('users.create_subtitle') }}</p>
        </div>
                ...
                <div>
                    ...
                    <div class="mb-6">
                        <label for="phone" class="block text-sm font-semibold text-secondary mb-2">
                            {{ __('users.fields.phone') }} <span class="text-red-500">*</span>
                        </label>
                        <input
                            type="text"
                            name="phone"
                            id="phone"
                            value="{{ old('phone') }}"
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary transition-all @error('phone') border-red-500 @enderror"
                ...
                <div>
                    ...
                    <div class="mb-6">
                        ...
                        <select 
                            ...
                            @foreach($roles->filter(fn ($role) => ! \App\Models\Role::matches($role->name, \App\Models\Role::ADMIN)) as $role)
                    ...
                    <div class="mb-6">
                        ...
                        <select
                            ...
                            required>
                            <option value="full" {{ old('login_type') == 'full' ? 'selected' : '' }}>Full (management only)</option>
                            <option value="both" {{ old('login_type') == 'both' ? 'selected' : '' }}>Both (management + passkey)</option>
                            <option value="staff" {{ old('login_type') == 'staff' ? 'selected' : '' }}>Staff (passkey only)</option>
                        </select>
                        @error('login_type')
                            <p class="mt-1.5 text-sm text-red-600 flex items-center gap-1">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div class="mb-6">
                        ...
                        <p class="mt-1.5 text-xs text-gray-500">Required for staff passkey login. Only numbers, 4 digits.</p>
                        @error('passkey')
                            <p class="mt-1.5 text-sm text-red-600 flex items-center gap-1">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
